...
void myMerge(int* array,int* temp,int low,int high)
{
	if (low >= high) return;
	int mid = low + (high - low) / 2;
	myMerge(array,temp,low,mid);
	myMerge(array,temp,mid + 1,high);
...
void myMergeTest()
{
	cout << "--------------------myMergeTest-------------------" << endl;
	int a[] = {5,3,1,7,5,3,2,0,9};
	for (int i = 0;i < sizeof(a)/sizeof(a[0]);i++)
		cout << a[i] << " ";
	cout << endl;
	int* b = new int[sizeof(a)/sizeof(a[0])];
	cout << "after merge: " << endl;
	myMerge(a,b,0,sizeof(a)/sizeof(a[0]) - 1);
	for (int i = 0;i < sizeof(a)/sizeof(a[0]);i++)
		cout << a[i] << " ";
	cout << endl;
	cout << "----------------------END------------------------" << endl;
